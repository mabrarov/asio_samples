  ...
  {  
    ...
    {  
      ...
      public:      
        ...
        {
          pointer ptr(new this_type(
            io_service, 
            read_buffer_size, 
            read_message_buffer_capacity, 
        ...
        {        
          strand_.dispatch(make_context_alloc_handler(handler,
            boost::bind(&this_type::write<Handler>, pointer(this), message, boost::make_tuple(handler))));
        }
        ...
        {        
          strand_.dispatch(make_context_alloc_handler(handler,
            boost::bind(&this_type::read<Handler>, pointer(this), boost::ref(message), boost::make_tuple(handler))));
        }        
      ...
      private:
        ...
        void shutdown(boost::tuple<Handler> handler)
        {
          ...
          {
            // Write in progress - store handler and wait for write operation completion          
        ...
        void handle_write(const boost::system::error_code& error, const std::size_t bytes_transferred)
        {
          ...
          if (!closed_)
          {
            ...
            if (shutdown_handler_ && !write_handler_ && !writing_)
            {
              // Shutdown in progress 
              // && no user write operations 
              // && no internal write operations 
