...
if (${env:COVERAGE_BUILD} -eq "True") {
  ...
  Write-Host "Calling: codecov --required --token ""${env:CODECOV_TOKEN}"" --file ""${env:COBERTURA_COVERAGE_FILE}"" --flags ""${env:CODECOV_FLAG}"" -X gcov --root ""${env:APPVEYOR_BUILD_FOLDER}"""
