...
typedef ma::echo::server::endpoint_type endpoint_type;
typedef boost::function<void (void)> exception_handler_type;

struct server_fsm : private boost::noncopyable
{
  boost::mutex mutex_;
  boost::condition_variable stop_condition_;
  ma::echo::server::pointer server_;
  boost::asio::deadline_timer stop_timer_;
  bool stop_in_progress_;
  bool stopped_;  

  explicit server_fsm(boost::asio::io_service& server_io_service,
    boost::asio::io_service& session_io_service)
    : server_(new ma::echo::server(server_io_service, session_io_service))
...
typedef boost::shared_ptr<server_fsm> server_fsm_ptr;

void on_server_start(const server_fsm_ptr&,
  const boost::system::error_code&);

void on_server_serve(const server_fsm_ptr&,
  const boost::system::error_code&);

void on_server_stop(const server_fsm_ptr&,
  const boost::system::error_code&);

void on_server_stop_timeout(const server_fsm_ptr&,
  const boost::system::error_code&);

void on_console_close(const server_fsm_ptr&);

void run_io_service(boost::asio::io_service&, exception_handler_type);

void on_exception(const server_fsm_ptr&);
...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  else
  {
    ...
    server_fsm_ptr server_fsm(new server_fsm(session_manager_io_service, session_io_service));
    
    // Start the server
    server_fsm->server_->async_start(boost::bind(&on_server_start, server_fsm, _1));
    std::wcout << L"Server is starting.\n";
    
    // Setup console controller
    ma::console_controller console_controller(boost::bind(&on_console_close, server_fsm));

    std::wcout << L"Press Ctrl+C (Ctrl+Break) to exit.\n";

    // Exception handler for automatic server aborting
    exception_handler_type exception_handler(boost::bind(&on_exception, server_fsm));
    ...
    boost::unique_lock<boost::mutex> lock(server_fsm->mutex_);
    if (!server_fsm->stopped_)
    {
      server_fsm->stop_condition_.wait(lock);
    }
    lock.unlock();
...
void on_exception(const server_fsm_ptr& server_fsm)
{
  boost::unique_lock<boost::mutex> lock(server_fsm->mutex_);
  server_fsm->stopped_ = true;
  std::wcout << L"Server work is interrupted due to to unexpected exception.\n";
  lock.unlock();
  // Notify main thread
  server_fsm->stop_condition_.notify_all();    
}

void on_console_close(const server_fsm_ptr& server_fsm)
{
  boost::unique_lock<boost::mutex> lock(server_fsm->mutex_);
  if (server_fsm->stop_in_progress_)
  {
    std::wcout << L"User console close detected. Server is already stopping.\n";
  }
  else if (server_fsm->stopped_)
  {    
  ...
  {
    // Start server stop
    server_fsm->server_->async_stop(
      boost::bind(&on_server_stop, server_fsm, _1));
    // Set up stop timer
    server_fsm->stop_timer_.expires_from_now(stop_timeout);
    server_fsm->stop_timer_.async_wait(
      boost::bind(&on_server_stop_timeout, server_fsm, _1));
    // Remember server state
    server_fsm->stop_in_progress_ = true;   
    std::wcout << L"User console close detected. Server is stopping.\n";    
  }  
}

void on_server_start(const server_fsm_ptr& server_fsm,
  const boost::system::error_code& error)
{   
  boost::unique_lock<boost::mutex> lock(server_fsm->mutex_);
  if (!server_fsm->stop_in_progress_ && !server_fsm->stopped_)
  {   
    if (error)
    {    
      // Remember server state
    ...
    else
    {
      ...
      server_fsm->server_->async_serve(
        boost::bind(&on_server_serve, server_fsm, _1));
      std::wcout << L"Server has started.\n";
    }
  }  
}

void on_server_serve(const server_fsm_ptr& server_fsm,
  const boost::system::error_code&)
{
  boost::unique_lock<boost::mutex> lock(server_fsm->mutex_);
  if (!server_fsm->stop_in_progress_ && !server_fsm->stopped_)
...
void on_server_stop(const server_fsm_ptr& server_fsm,
  const boost::system::error_code&)
{
  boost::unique_lock<boost::mutex> lock(server_fsm->mutex_);
  if (!server_fsm->stopped_ && server_fsm->stop_timer_.cancel())
  {  
    // Remember server state
    server_fsm->stopped_ = true;
...
void on_server_stop_timeout(const server_fsm_ptr& server_fsm,
  const boost::system::error_code& error)
{
  boost::unique_lock<boost::mutex> lock(server_fsm->mutex_);
  if (!server_fsm->stopped_ && !error)
  {    
    // Remember server state
    server_fsm->stopped_ = true;
